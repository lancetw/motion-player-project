...
//	bitmap_offset = conv_b2l((void*)buf, sizeof(int32_t)); // ビットマップオフセット
	...
	for(k = 0;k < 48;k++){ // サンプリングRAM初期化
		*(uint32_t*)&glyph_samples[k][0] = 0x00000000;
		*(uint32_t*)&glyph_samples[k][4] = 0x00000000;
		*(uint32_t*)&glyph_samples[k][8] = 0x00000000;
	}

//	misc = 0;
	float fypos, ratioX0, ratioX1, ratioY0, ratioY1;
	pixel_fmt_typedef pixel_fgBox[2][2], pixel_bgBox[2][2], pixelV1, pixelV2, pixel;

	u32_p = (uint32_t*)buf;
	for(i = 0;i < metric.character_ascent + metric.character_descent;i++){
		bitmap_data_64 = __REV(*u32_p++);
		/*
		temp = *((uint32_t*)&buf[misc]);
		misc += 4;
		bitmap_data = temp;
		bitmap_data <<= 8;
		bitmap_data |= (uint8_t)(temp >> 8);
		bitmap_data <<= 8;
		bitmap_data |= (uint8_t)(temp >> 16);
		bitmap_data <<= 8;
		bitmap_data |= (uint8_t)(temp >> 24);
	...
	misc = 11 - (int)((float)(metric.character_ascent / 4.0f) + 0.5f); // グリフの高さ揃えパラメータ
	/*
...
*/
	for(k = 0;k < 12;k++){ // 背景データをRAMに格納
		temp = k + cly + misc;
		LCDSetGramAddr(clx, temp < LCD_HEIGHT ? temp : LCD_HEIGHT - 1);
		LCDPutCmd(0x0022);
		LCD->RAM; // dummy read
		for(j = 0;j < 13;j++){
			bg_ram[k][j] = LCD->RAM;
...
*/
	...
	clx += metric.right_sided_bearing / 4 - metric.left_sided_bearing / 4 + pcf_font.metrics.hSpacing;
...
//	bitmap_offset = conv_b2l((void*)buf, sizeof(int32_t)); // ビットマップオフセット
	...
	for(k = 0;k < 48;k++){ // サンプリングRAM初期化
		...
		*(uint32_t*)&glyph_samples[k][4] = 0x00000000;
		*(uint32_t*)&glyph_samples[k][8] = 0x00000000;
		*(uint32_t*)&glyph_samples[k][12] = 0x00000000;
	}
...
*/
	...
	misc = 11 - (int)((float)metric.character_ascent / 3.0f + 0.5f); // グリフの高さ揃えパラメータ
	...
	clx += (metric.right_sided_bearing / 3 - metric.left_sided_bearing / 3 + pcf_font.metrics.hSpacing);
	if(code == 0x20 || code == 0x3000){
		clx += 5;
	}
}
...
void PCFPutCharCache(uint16_t code, colors color)
{
	...
	misc = 11 - (int)((float)metric.character_ascent / (font_width < 16 ? 4.0f : 3.0f) + 0.5f); // グリフの高さ揃えパラメータ
	...
	clx += metric.right_sided_bearing / (font_width < 16 ? 4 : 3) - metric.left_sided_bearing / (font_width < 16 ? 4 : 3) + pcf_font.metrics.hSpacing;
}
...
//	metric.character_ascent    ^= (1 << 7);
	...
	len = metric.right_sided_bearing / (font_width < 16 ? 4 : 3) - metric.left_sided_bearing / (font_width < 16 ? 4 : 3) + pcf_font.metrics.hSpacing;
